...
export function Navbar({ wordmark = null }: { wordmark?: Wordmark | null }) {
  ...
  return (
    /*
      ...
      Once scrolled the header carries `backdrop-blur`, and a backdrop-filter
      makes an element a containing block for fixed-positioned descendants. As
      a child, the panel's `fixed inset-0` resolved against the 72px header
      instead of the viewport, so the menu collapsed into the bar on every
      scrolled page. Rendering it outside keeps `fixed` meaning the viewport.
    */
    <>
      <header
        className={cn(
